...
inline auto openapi_check_server(const JSON &value, const Pointer &base)
  ...
  if (variables != nullptr) {
    ...
    for (const auto &entry : variables->as_object()) {
      openapi_check_server_variable(entry.second,
                                    openapi_child(variables_base, entry.first));
    }
  }
}
...
inline auto openapi_check_servers(const JSON &document) -> void {
  const auto *servers{document.try_at("servers", OPENAPI_HASH_SERVERS)};
  if (servers == nullptr) {
    return;
  }

  const Pointer base{"servers"};
  if (!servers->is_array()) {
    throw OpenAPIError{base,
  ...
  for (const auto &server : servers->as_array()) {
    openapi_check_server(server, openapi_child(base, index));
    index += 1;
  }
}
