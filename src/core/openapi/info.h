...
inline auto openapi_parse_info(const JSON &document) -> OpenAPIInfo {
  ...
  if (contact != nullptr) {
    result.contact =
        openapi_parse_contact(*contact, openapi_child(base, "contact"sv));
  }

  const auto *license{value->try_at("license", OPENAPI_HASH_LICENSE)};
  if (license != nullptr) {
    result.license =
        openapi_parse_license(*license, openapi_child(base, "license"sv));
  }
